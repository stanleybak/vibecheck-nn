...
- **Official VNNCOMP scoring of a `sat` (mirror it exactly).** The submitted input witness `X` is replayed with **onnxruntime on CPU** (ONNX 1.21.0 / onnxruntime 1.26.0); the solver's `Y` is **ignored** — the authoritative output is ORT's recomputed `Y`. A witness is CORRECT iff (a) `X` is inside the VNNLIB input box within `tol` (`x < lb-tol or x > ub+tol` ⇒ outside) AND (b) the output spec is violated within `tol` (`prop_mat·Y ≤ prop_rhs + tol`). If it only holds because of the tolerance (needs `tol` and not the 1e-7 check) it scores CORRECT_UP_TO_TOLERANCE (accepted, no penalty; others may still prove `unsat`). `tol` = `COUNTEREXAMPLE_ATOL` = **1e-4**, exposed as the `sat_validate_atol` setting (configurable). **Our process, on EVERY PGD/CE path:** find a candidate → write the CE early (X = witness, Y = ORT-recomputed) → replay on CPU ORT → if it doesn't violate within `sat_validate_atol`, drop it and keep searching; if it's a clear CE return `sat` immediately; if only within-tol, keep the early CE but keep searching for a clear CE / `unsat`. Implemented as the `verify_graph._finalize` chokepoint (`_validate_sat_witness` + `_sat_disposition`, ORT-CPU, input-box ±atol + output-spec ±atol) AND a universal re-validation in `main._verify` before emitting `sat` (covers the milp/bnb/hybrid/conv→milp-auto-route verifiers, which don't validate internally). Never emit a `sat` whose witness ORT rejects.
