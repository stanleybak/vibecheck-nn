...
Known upstream benchmark bugs — work around, note in the README, don't burn time: unconverted v1 files inside 2.0/ dirs (safenlp 20,544/21,624; malbeware 325/375; linearizenn 60/120; cora 39/99; cersyve 6/9), malformed s-exprs in linearizenn `*_io`, broken instances.csv path for cgan small_transformer. **Stale `.gz` trap:** in some 2.0/ dirs the uncompressed `.vnnlib`/`.onnx` siblings are stale leftovers (older than, and differing from, the git-tracked `.gz`); only the `.gz` is authoritative — read/gunzip the `.gz`, not the loose file. (Our AWS harness `gunzip -kf` force-overwrites, so runs use fresh content; local parsing must not.)

**isomorphic_acasxu_2026/2.0 and monotonic_acasxu_2026/2.0 (corrected 2026-06-18, repo @889c98d2):** earlier notes calling these "buggy" (contradictory `and`, invalid infix asserts, broken `onnx/original/` paths) are STALE — upstream reformatted both into clean, well-formed **v2 two-network** specs (`declare-network f` + `declare-network g (isomorphic-to f)`/`(equal-to f)`, cross-network `(== X_f[i] X_g[i])`, and for monotonic the relational `(>= X_f[0] X_g[0])`; isomorphic output is a proper `(or |Y_g[i]-Y_f[i]|>0.05)`, monotonic output `(< Y_f[3] Y_g[3])`). All onnx paths resolve. ABC's 2025 single-`--onnx_path` harness + loader can't ingest these multi-network v2 specs → they score `unknown` for the ABC baseline (correct). Solvable in **vibecheck** via a merged graph (f‖g sharing/relating inputs) + v2 multi-network parsing; monotonic's relational input maps to a v1 box via the `δ`+`Clip` parametrization.
