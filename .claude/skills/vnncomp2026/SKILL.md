...
- Benchmarks marked "old" with an existing `docs/benchmarks/<name>.md` and matching config need only a **confirmation sweep** on the 2026 instance set; re-open them only if the instances changed or the sweep regresses.

Benchmarks live at `~/repositories/vnncomp2026_benchmarks` locally — **rsync to AWS before first use**. Measurements run on AWS (see CLAUDE.md for hosts, idle_since discipline, keepalive, verdict-file rules).
